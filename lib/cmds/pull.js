...
const { getAllFiles, addCommmitMsgPrefix, getTempRepo, copyFiles, deleteFiles } = require("../utils");
...
async function updateFolderFromMain(currentDir) {
  try {
    ...
    const mainRepoPath = await getTempRepo(currentDir, config.repoUrl, config.branch);
