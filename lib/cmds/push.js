...
async function updateMainFromFolder(
  ...
  try {
    ...
    console.log(currentDir, '===current directory')
    console.log(folderRepo, '===main directory')
    ...
    const refs = await mainRepo.getReferenceNames(Git.Reference.TYPE.LISTALL)
    ...
    const list = await fs.readdir(mainRepoPath)
    console.log('mainRepo lst: ', list)

    console.log('current')
    console.log(
      fs.readFileSync(
        '/home/runner/work/gitslice-ci/gitslice-ci/.env.test',
        'utf8'
      )
    )
    console.log('main')
    console.log(fs.readFileSync(mainRepoPath + '/.env.test'))
