...
async function updateMainFromFolder(
  ...
  try {
    ...
    if (
      ...
      console.log('after folderRepo checkoutBranch', curBranchName)
      await mainRepo.createBranch(
        branchName,
        commitHash,
        0 // gives error if the branch already exists
      )
      console.log('after createBranch checkoutBranch', curBranchName)
    } else {
    ...
    console.log('after getStatus', repoStatus)
    if (repoStatus.length) {
      ...
      console.log('after createCommit')
