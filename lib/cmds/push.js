...
async function updateMainFromFolder(
  ...
  try {
    ...
    if (!!clientRepoDetails) {
      ...
      clientBaseBranchName = clientRepoDetails.clientBaseBranchName || config.branch
    }
    ...
    if ((await mainRepo.getReferenceNames(Git.Reference.TYPE.LISTALL)).indexOf(`refs/heads/${branchName}`) !== -1) {
      console.log('Branch already exists, will attempt to pull from remote');
    }
    ...
    const repoAhead = gitState.aheadSync(mainRepoPath)
