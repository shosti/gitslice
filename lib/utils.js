...
async function isBranchExist(repo, branchName, type) {
  try {
    const { Branch } = Git;
    const { BRANCH } = Branch;
    const branchType =  type || BRANCH.LOCAL
    // { LOCAL: 1, REMOTE: 2, ALL: 3 }
    const branch = await Branch.lookup(repo, branchName, branchType);

    return branch;
  } catch (error) {};
};

async function createBranchIfNotExist(repo, branchName, commitHash) {
  const branchExist = await isBranchExist(repo, branchName);
...
async function createOrPullBranch(repo, branchName, commitHash) {
  try {
    let createdBranch = await createBranchIfNotExist(repo, branchName, commitHash);

    createdBranch = createdBranch || await repo.mergeBranches(branchName, 'refs/remotes/origin/' + branchName);
