...
async function cloneRepo(mainUrl, mainRepoPath) {
  try {
    ...
    return Promise.reject(`Unable to clone ${mainUrl}`);
  }
}

async function updateFromOrigin(repo, branch) {
  try {
    ...
    return Promise.reject(`Unable to update ${mainUrl}`);
  }
}
...
async function getTempRepo(mainUrl, branch) {
  ...
  try {
    ...
    if (origin.url() === mainUrl) {
      await updateFromOrigin(repo, branch);
    } else {
      await cloneRepo(mainUrl, mainRepoPath);
    }
  } catch (e) {
...
async function pushTempRepo(repoUrl, branch) {
  const mainRepoPath = getTempRepoPath(url);
  const repo = await Git.Repository.open(mainRepoPath);
  const origin = await repo.getRemote("origin");
