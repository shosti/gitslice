...
async function getLastGitSliceCommitHash(repo) {
  ...
  return removeCommitMsgPrefix(
    commits.find(x => x.message().indexOf("git-slice:") === 0).message()
  );
}
...
async function copyFiles(source, destination, folders, ignored) {
  ...
  for (let p of folders) {
    ...
    for (let sourceFile of allFiles) {
      const isIgnored = await Git.Ignore.pathIsIgnored(repo, sourceFile);
      if (!isIgnored && ignoredFiles.indexOf(sourceFile) === -1) {
...
async function deleteFiles(path, ignored) {
  const repo = await Git.Repository.open(path);
  const ignoredFiles = ignored.map(f => path.resolve(path, f));
  for (let file of await getAllFiles(path)) {
    const isIgnored = await Git.Ignore.pathIsIgnored(repo, sourceFile);
    if (!isIgnored && ignoredFiles.indexOf(sourceFile) === -1) {
