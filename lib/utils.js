...
async function getTempRepo(params) {
  const { mainUrl, branch, username, password, testSuite } = params
  if (username && password) {
    _username = username
    _password = password
  }
  const mainRepoPath = getTempRepoPath(testSuite)
  try {
...
function getTempRepoPath(url) {
  return path.join(os.tmpdir(), 'git-slice', url)
}

async function pushTempRepo(repoUrl, branch) {
  try {
    const mainRepoPath = getTempRepoPath('push')
    const repo = await Git.Repository.open(mainRepoPath)
    const origin = await repo.getRemote('origin')
