...
const isSourceInIgnoredDir = async (ignored, sourceFile) =>
  ignored.some((ignore) => sourceFile.startsWith(ignore))
...
async function copyFiles(source, destination, folders, ignored) {
  ...
  for (let p of folders) {
    ...
    for (let sourceFile of allFiles) {
      ...
      const isInIgnoredDir = await isSourceInIgnoredDir(
        ignored,
      ...
      if (
        !isGitIgnored &&
        ignoredFiles.indexOf(sourceFile) === -1 &&
        !isInIgnoredDir
      ) {
...
async function deleteFiles(source, ignored) {
  ...
  for (let file of await getAllFiles(source)) {
    ...
    if (!isGitIgnored && ignoredFiles.indexOf(file) === -1 && !isInIgnoredDir) {
