...
function setGitCredentials(url, username, password) {
  return new Promise((resolve, reject) => {
    gitCredentialHelper.approve(url, resolve, {
      username,
      password
    })
  })
...
async function promptForCredentials(url, existingUserName) {
  console.log('getting credentials', url)
  let { username, password } = await getGitCredentials(url)
  ...
  console.log('got credentials', username)
...
async function getTempRepo(mainUrl, branch, username, password) {
  ...
  try {
    ...
    if (username && password) {
      ...
      await setGitCredentials(mainUrl, username, password)
    }
