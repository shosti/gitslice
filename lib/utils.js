...
async function createOrPullBranch(repo, branchName, commitHash) {
  try {
    if (
      (await repo.getReferenceNames(Git.Reference.TYPE.LISTALL)).indexOf(
        `refs/heads/${branchName}`
      ) !== -1
    ) {
      console.log(
        'Branch already exists, will attempt to update branch from remote first'
      )
      pullBranch(mainRepo, branchName)
    } else {
...
async function pullBranch(repo, branchName) {
  try {
    return repo.mergeBranches(branchName, 'refs/remotes/origin/' + branchName)
  } catch (error) {}
}
