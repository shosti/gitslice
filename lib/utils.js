    ...
    for (let sourceFile of allFiles) {
      ...
      if (!isGitIgnored && ignoredFiles.indexOf(sourceFile) === -1) {
        ...
        if (stats.isSymbolicLink()) {
          const symbolicLinkTargetSourceFile = path.resolve(
            sourceFile,
            nodeFs.readlinkSync(sourceFile)
          )
          const symbolicLinkTargetDesFile = symbolicLinkTargetSourceFile.replace(
            source,
            destination
          )
          await fs.ensureFile(symbolicLinkTargetDesFile)
          if (await fs.pathExists(symbolicLinkTargetSourceFile)) {
            await fs.copy(
              symbolicLinkTargetSourceFile,
              symbolicLinkTargetDesFile
            )
            const desFile = sourceFile.replace(source, destination)
            await fs.ensureFile(desFile)
            nodeFs.symlinkSync(desFile, nodeFs.readlinkSync(sourceFile))
          }
        } else {
          const desFile = sourceFile.replace(source, destination)
          await fs.ensureFile(desFile)
