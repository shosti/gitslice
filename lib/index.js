  ...
  try {
    ...
    switch (command) {
      ...
      case "pull":
        ...
        break;
      case "push":
        if (argv.branch && argv.message && argv.authorName && argv.authorEmail)  {
          await updateMainFromFolder(currentDir, argv.branch, argv.message, argv.authorName, argv.authorEmail);
        } else {
          console.log(INVALID_ARG_MSG);
          showHelp(command);
        }
