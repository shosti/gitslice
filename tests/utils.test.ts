...
import parseArgsAndExecute from '../src/cmds';
import {
  getAllFiles,
  getCurBranch,
  addCommmitMsgPrefix,
...
import beforeHook from './helpers/before';
...
let folderRepo: Git.Repository;
let mainRepo: Git.Repository;
...
beforeEach(async function() {
  this.timeout(10000)
  const { main, folder } = await beforeHook(folderRepoPath)
  mainRepo = main
  folderRepo = folder
...
describe('error tests', () => {
  it('on error process should return error code 1', async () => {
    ...
    expect(processExitStub.calledWith(1)).toEqual(true)
  })
})
